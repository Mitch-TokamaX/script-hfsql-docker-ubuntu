...
# Déployer [HFSQL](https://pcsoft.fr/accueilpub/hfsql.htm) sur [AWS Lightsail](https://aws.amazon.com/fr/lightsail/) avec [Docker](https://www.docker.com/) (pour ~6€/mois)
...
Le guide suppose de disposer préalablement d’un [compte AWS](https://aws.amazon.com/fr/account/) et des [droits nécessaires](https://docs.aws.amazon.com/fr_fr/lightsail/latest/userguide/amazon-lightsail-managing-access-for-an-iam-user.html) pour créer une instance Lightsail.

## Avertissements

- Ce document est fourni à des fins pédagogiques.
- Vérifier la configuration réseau et les règles de firewall avant
  une mise en production.
- Adapter le mot de passe, le port et les permissions de stockage aux
  contraintes de sécurité.
- Les coûts Lightsail, les snapshots et les options réseau peuvent
  évoluer : contrôler toujours la console AWS avant validation.
- S'il ne s'agit que d'utiliser ce script à des fins d'évaluation, penser à stopper ou à détruire l'instance LightSail HFSQL pour éviter une facturation inutile.
...
Depuis la console AWS, ouvrir le module Lightsail. La liste des éventuelles instances existantes s’affiche. Cliquer ensuite sur **Create instance** pour démarrer la création du futur serveur HFSQL.
...
La zone de création de l’instance apparaît. Ajuster au besoin la région et la zone de disponibilité.

| ![Choix de la région et de la zone](images/02-region-and-zone.png) |
:--- |


Choisir une instance de type Linux, puis « Operating System only », puis Ubuntu 24.04.

| ![Sélection de Linux et d’Ubuntu 24.04](images/03-ubuntu-24-04-selection.png) |
:--- |


Dans la partie Optional, cliquer sur Add launch script.

| ![Ajout du launch script](images/04-add-launch-script.png) |
:--- |

Coller le script fourni à la fin de ce tutoriel.

**Renseigner immédiatement le mot de passe HFSQL dans la variable**
...
> **Attention**
>
> - Le script est conçu pour Ubuntu. Si une autre distribution est choisie, il faudra l'adapter.
...
Choisir ensuite la taille de l’instance. Pour un GDS ou une petite base HFSQL, le premier niveau de gamme à 5$/mois est souvent suffisant pour un usage à coût très maîtrisé, mais le script d'installation est plutôt long à s'exécuter. Ici pour 2$ de plus on s'offre un peu plus de confort. *Les bases GDS du Groupe Tokamax sont toutes hébergées sur LightSail depuis plusieurs années sans aucun problème de temps de réponse ou de disponiblité.*

| ![Choix de la taille de l’instance](images/07-instance-size.png) |
:--- |

Renseigner enfin le nom de l’instance puis lancer sa création.
...
Passer dans l’onglet Networking pour revoir les règles de firewall.

| ![État initial du firewall IPv4](images/10-initial-ipv4-firewall.png) |
:--- |

Par défaut, le port 22 est utile pour SSH : le conserver. Le port 80 apparaît dans la configuration initiale ; si on n'en a pas besoin, on peut le retirer ici.

_________________________________________________________________

## Étape 5 — Ouvrir le port HFSQL

Ajouter ensuite une règle Custom / TCP correspondant au port externe défini dans le script, 4900 dans l’exemple ci-dessous.
...
> - En production, limiter la règle aux IP autorisées.
> - Si HFSQL_EXTERNAL_PORT a été modifié dans le script, ouvrir ce même
...
Par défaut, l’adresse IP publique d’une instance Lightsail peut changer après un arrêt, un redémarrage ou une recréation du serveur. Pour disposer d’une adresse stable, attacher une IP statique.

| ![Adresse IP publique avant IP statique](images/13-public-ip-before-static-ip.png) |
:--- |

Donner un nom à l’adresse IP puis valider l’association.

| ![Création et attachement de l’IP statique](images/14-create-and-attach-static-ip.png) |
:--- |

Une fois l’IP statique attachée, l’utiliser telle quelle ou lui associer un nom DNS (pratique pour accéder au serveur HFSQL en utilisant *hfsql.acme.com*).
...
Ouvrir l’onglet Connect puis cliquer sur Connect using SSH. Cela ouvre la console navigateur de Lightsail.

| ![Ouverture de la console SSH](images/16-open-ssh-console.png) |
:--- |

Première vérification utile : attendre la fin du *launch script*, puis afficher les conteneurs Docker actifs.

Si la configuration réseau a été très rapide, il est possible que le script d’installation soit encore en cours d’exécution. Dans ce cas, attendre encore quelques instants avant de relancer les vérifications.
...
Pour diagnostiquer un démarrage plus difficile, garder ces commandes sous la main : 
...
Le serveur est maintenant prêt. Ouvrir le Centre de contrôle HFSQL puis renseigner l’adresse IP statique — ou le nom DNS —, le port choisi et le mot de passe défini dans la variable HFSQL_PASSWORD du script.

| ![Connexion au serveur depuis le Centre de contrôle HFSQL](images/17-hfsql-control-center-connection.png) |
:--- |

Une fois la connexion établie, on peut créer une base GDS, restaurer une base existante ou connecter directement une application WinDev.
...
- Si le conteneur est recréé, conserver le même montage pour retrouver
  les bases.
...
>
> - Remplacer **HFSQL_PASSWORD** par un mot de passe fort.
> - Vérifier la valeur de **HFSQL_IMAGE_VERSION** (voir [Docker Hub windev/hfsql tags](https://hub.docker.com/r/windev/hfsql/tags))
> - Adapter le port si on ne souhaite pas exposer le 4900.
